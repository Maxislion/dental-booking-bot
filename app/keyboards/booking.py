        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🦷 Лечение кариеса", callback_data="service_caries"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🪥 Удаление зуба", callback_data="service_remove"
                )
            ],
...
def dates_kb(offset: int = 0):
    ...
    for i in range(0, len(dates), 2):
        row = []
        row.append(
            InlineKeyboardButton(text=dates[i], callback_data=f"date_{dates[i]}")
        )
    ...
    for i in range(0, len(times), 2):
        ...
        for t in times[i : i + 2]:
            if t in booked_times:
                ...
                row.append(InlineKeyboardButton(text=f"❌ {t}", callback_data="busy"))
            else:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                ...
                InlineKeyboardButton(
                    text="❌ Отменить",
                    callback_data=f"admin_reject|{user_id}|{doctor}|{date}|{time}",
                ),
            ]
        ]
    )
